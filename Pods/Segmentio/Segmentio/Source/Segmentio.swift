...
open class Segmentio: UIView {
    ...
    private func moveShapeLayerAtContext() {
        let itemWitdh = segmentioItems.enumerated().map { (index, _) -> CGFloat in
            segmentWidth(for: IndexPath(item: index, section: 0))
        }
    ...
    private func segmentWidth(for indexPath: IndexPath) -> CGFloat {
        ...
        case let .fixed(maxVisibleItems):
...
extension Segmentio.Points {
    init(item: Segmentio.ItemInSuperview, atIndex index: Int, allItemsCellWidth: [CGFloat], pointY: CGFloat, position _: SegmentioPosition, style _: SegmentioStyle, insets: UIEdgeInsets, isCommonBehaviour: Bool) {
